...
class HybridGRPOTrainer(Trainer):
    """
    ...
    def _generate_and_score_completions(
        ...
        task = [example["task"] for example in inputs]
        task = gather_object(task)
        
        # Log the metrics
        mode = "eval" if self.control.should_evaluate else "train"

        completion_length = self.accelerator.gather_for_metrics(completion_mask.sum(1))
        for t, cl in zip(task, completion_length):
            self._metrics[mode][f"{t}_completion_length"].append(cl.item())
        ...
        for t, r, r_std in zip(task, rewards, std_grouped_rewards):
            self._metrics[mode][f"{t}_reward"].append(r.item())
            self._metrics[mode][f"{t}_reward_std"].append(r_std.item())
