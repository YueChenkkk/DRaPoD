...
class GRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(
        ...
        prompts_difficulty = gather_object([x["difficulty"] for x in inputs])
        prompts_difficulty = prompts_difficulty[::self.num_generations]
        ...
        # log rewards for different difficulty levels
        for dfc, mean_rwd, std_rwd in zip(prompts_difficulty, mean_grouped_rewards, std_grouped_rewards):
            self._metrics[mode][f"{dfc}_mean_reward"].append(mean_rwd.item())
            self._metrics[mode][f"{dfc}_std_reward"].append(std_rwd.item())
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        per_token_logps = torch.zeros(prompt_ids.size(0), logits_to_keep, device=input_ids.device)
        for i, (pc_len, pc) in enumerate(zip(inputs["prompt_completion_lengths"], input_ids)):
            logps = self._get_per_token_logps(
                model, pc[:pc_len].unsqueeze(0), None, pc_len - prompt_ids.size(1)
            )
            per_token_logps[i][:pc_len - prompt_ids.size(1)] = logps[0]
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        mode = "eval" if self.control.should_evaluate else "train"
        metrics = {key: sum(val) / len(val) if len(val) > 0 else 0 for key, val in self._metrics[mode].items()}  # average the metrics
    ...
    def evaluate(
        ...
        output.metrics.update({f"eval_{key}": sum(val) / len(val) for key, val in self._metrics["eval"].items()})
